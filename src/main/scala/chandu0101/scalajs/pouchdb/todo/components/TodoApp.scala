package chandu0101.scalajs.pouchdb.todo.components

import chandu0101.scalajs.pouchdb.todo.model.TodoItem
import chandu0101.scalajs.pouchdb.todo.services.TodoService
import chandu0101.scalajs.pouchdb.todo.util.PouchDBChangesListener
...
object TodoApp {
  ...
  val component = ReactComponentB[Unit]("TodoApp")
    ...
    .render((P, S, B) => {
      div(cls := "todoapp",
      input(marginLeft := "60px",tpe := "text",onKeyUp ==> B.handleItemSave),
      todoList((S,B))
      )
    })
